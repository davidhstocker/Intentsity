...
# Add the project root to sys.path so package modules are importable
    ...
    for eachReadLine in resAllLines:
        ...
        try:
            ...
            for actionCommand in stringRawArray:
                actionInvocation = Engine.ActionRequest(actionCommand, Engine.ActionInsertionType.APPEND, rtparams, agentID, agentID, controllerID)
        ...
        Graph.logQ.put( [logType , logLevel.INFO , method , f"Finished testcase {n}: {actionInvocation.actionMeme.fullTemplatePath}"])
    #Graph.logQ.put( [logType , logLevel.DEBUG , method , "exiting"])
    return resultSet 
...
def testActionQueueSingleAgent(resultFile, rawDataFile, agentMeme, parentAction = False, port = None):
    ...
    try:
        ...
        errorMessage = f"Unable to create entity from testcase meme {agentMeme}.  WTraceback = {e}"
    ...
    for eachReadLine in resAllLines:
        ...
        try:
            ...
            for actionCommand in stringRawArray:
                actionInvocation = Engine.ActionRequest(rmlEngine.api, actionCommand, Engine.ActionInsertionType.APPEND, rtparams, agentID, agentID, None)
                #debug
        ...
        Graph.logQ.put( [logType , logLevel.INFO , method , f"Finished testcase {n}"])
    #Graph.logQ.put( [logType , logLevel.DEBUG , method , "exiting"])
    return resultSet 
...
def testActionEngineDynamicLandmarks(rawDataFile, port = None):
    ...
    for rawLine in rawAllLines:
        ...
        try:
            ...
            actionInvocation = Engine.ActionRequest(rmlEngine.api, actionCommand, Engine.ActionInsertionType.APPEND, rtparams, agentID, agentID, controllerID, dynamicLandmarkID)
            rmlEngine.aQ.put(actionInvocation)
        ...
        testcase = f"{agentMeme}, {actionCommand}, {dynamicLandmark}"
        results = [n, testcase, testResult, expectedResult, ""]
        resultSet.append(results)
        
        Graph.logQ.put( [logType , logLevel.INFO , method , f"Finished testcase {n}: {actionCommand}"])
    #Graph.logQ.put( [logType , logLevel.DEBUG , method , "exiting"])
    return resultSet 
...
def testStimulusEngineTrailer():
    ...
    try:
        ...
        Graph.logQ.put( [logType , logLevel.DEBUG , method , f"Error testing trailer.  Traceback = {e}"])
        ...
        for agentID in agents:
            ...
            for result in resultList:
                if (len(resultList) > 1) and (result == "***"):
                    ...
                    testResult = f"{testResult}{result}"
                
            agentMemePath = rmlEngine.api.getEntityMemeType(agentID)
            testcase = f"Stimulus= {stimulusMemePath}, Agent={agentMemePath}"
...
def testStimulusEngine2(filenameAgentList, filenameTestcase, restrictAgents = True):
    ...
    for eachReadLineT in allLinesT:
                    ...
                    try:
                        ...
                        if standingBadResultSet == myAgentSet:
                            ...
                            if bool(result.agentSet) == True:
                                ...
                                testResult = False
                                notes = f"{notes}\nAgents with descriptor {resultDescriptor},Should be empty, but contain {list(result.agentSet)}"
                        elif myAgentSet.issubset(result.agentSet) == False:
                            testResult = False
                            notes = f"{notes}\nAgents with descriptor {resultDescriptor},{myAgentList}not a subset of {list(result.agentSet)}"
                        ...
                        if resultDescriptor != "***":
                            try:
                                ...
                                if badResultset.issubset(result.agentSet) == True:
                                    testResult = False
                                    notes = f"{notes}\nAgents with descriptor {resultDescriptor},{badResultset}not a subset of {list(result.agentSet)}"
                            except KeyError:
                                # this is ok
                                pass
                            except Exception as e:
        ...
        agentMemePath = rmlEngine.api.getEntityMemeType(agentID)
        testcase = f"Stimulus= {stimulusMemePath}"
...
def testDescriptorSimpleDirect():
    ...
    for eachReadLine in allLines:
        ...
        n = n+1
        splitTestData = re.split(r'\|', eachReadLine)
        ...
        try:
            ...
            Graph.logQ.put( [logType , logLevel.ERROR , method , f"Error testing testDescriptorSimpleDirect.  Traceback = {errorMsg}"])
    return resultSet
...
def testServerAPIServerUp(serverURL = None, callbackTestServerURL = None):
    ...
    if (serverURL is None) or (callbackTestServerURL is None):
        ...
        try:
            ...
            testResult = False
            print (f"Intentsity server at {statusSURL} is not available.  Skipping API tests")
        except Exception as e:
            raise e
        ...
        try:
            ...
            testResult = False
            print (f"Test Callback server at {statusCURL} is not available.  Skipping API tests")
        except Exception as e:
            raise e
    return testResult
...
def testServerAPIAdminStartup(expectedCode, serverURL = None, dbConnectionString = None, persistenceType = None, repoLocations = [[]],  validate = False):
    ...
    statusString = f"Expected Status: {expectedCodeS}, Status: {serverStartupStatus}"
...
def testServerAPICreateEntityFromMeme(serverURL = None, memePath = "Graphyne.Generic"):
    ...
    createEntityURL = serverURL + f"/modeling/createEntityFromMeme/{memePath}"
    notes = ""
    try:
        #urllib GET request
...
def testServerAPIGetEntityMemeType(serverURL = None, memePath = "Graphyne.Generic"):
    ...
    getEntityMemeTypeURL = serverURL + f"/modeling/getEntityMemeType/{entityUUIDJson['entityUUID']}"
    try:
        #urllib GET request
        ...
        if entityType != memePath:
            testResult = False
            notes = f"Meme type returned = {entityType}.  Should be {memePath}"
    except Exception as e:
        testResult = False
...
def testServerAPIGetEntityMetaMemeType(serverURL = None, memePath = "Graphyne.Generic", metaMemePath = "Graphyne.GenericMetaMeme"):
    ...
    createResponseJsonB = createResponse.read()
    entityUUIDJson = json.loads(createResponseJsonB)
    getEntityMetaMemeTypeURL = serverURL + f"/modeling/getEntityMetaMemeType/{entityUUIDJson['entityUUID']}"
    try:
        #urllib GET request
        ...
        if entityType != metaMemePath:
            testResult = False
            notes = f"Meta Meme type returned = {entityType}.  Should be {metaMemePath}"
    except Exception as e:
        testResult = False
...
def testServerAPIGetEntitiesByMemeType(serverURL = None, memePath = "Graphyne.Generic"):
    ...
    createEntityURL = serverURL + f"/modeling/createEntityFromMeme/{memePath}"
    getEntityURL = serverURL + f"/modeling/getEntitiesByMemeType/{memePath}"
    notes = ""
    entityID = None
    try:
        #urllib GET request
    ...
    if testResult != False:
        try:
            ...
            if entityID not in entityIDList:
                ...
                notes = f"Entity {entityID} was {entityIDListS}"
        except urllib.error.URLError as e:
            testResult = False
            notes = e.reason
        except Exception as e:
...
def testServerAPIGetEntitiesByMetaMemeType(serverURL = None, memePath = "Graphyne.Generic", metaMemePath = "Graphyne.GenericMetaMeme"):
    ...
    createEntityURL = serverURL + f"/modeling/createEntityFromMeme/{memePath}"
    getEntityURL = serverURL + f"/modeling/getEntitiesByMetaMemeType/{metaMemePath}"
    notes = ""
    entityID = None
    try:
        #urllib GET request
...
def testServerAPIGetAreEntitiesLinked(serverURL = None, memePath = "Graphyne.Generic"):
    ...
    if testResult != False:   
        #Link the two
        getLinkedURL = serverURL + f"/modeling/getAreEntitiesLinked/{entityUUID1Json["entityUUID"]}/{entityUUID2Json["entityUUID"]}"
        try:
            #urllib GET request
...
def testServerAPIRemoveEntityLink(serverURL = None, memePath = "Graphyne.Generic"):
    ...
    if testResult != False:   
        #Link the two
        removeEntityMemeTypeURL = serverURL + f"/modeling/removeEntityLink/{entityUUID1Json["entityUUID"]}/{entityUUID2Json["entityUUID"]}"
        try:
            #urllib GET request
...
def testServerAPIGetLinkCounterpartsByType(serverURL = None, fName = "Entity_Phase7.atest"):
    ...
    for eachReadLine in allLines:
        ...
        testResult = False
        try:
            createEntityURL0 = serverURL + f"/modeling/createEntityFromMeme/{stringArray}"[0]
            createEntityURL1 = serverURL + f"/modeling/createEntityFromMeme/{stringArray}"[1]
...
def testServerAPIAEntityPropertiesRead(serverURL = None, memePath = "Graphyne.Generic"):
    ...
    if testResult != False:
        try:
            ...
            getPropURL = serverURL + f"/modeling/getEntityPropertyValue/{entityID}/{"Hello"}"
...
def testServerAPIAEntityPropertiesPresent(serverURL = None, memePath = "Graphyne.Generic"):
    ...
    if testResult != False:
        try:
            #Now read that same property    
            getPropURL = serverURL + f"/modeling/getEntityHasProperty/{entityID}/{"Hello"}"
            readResponse = urllib.request.urlopen(getPropURL) 
            readResponseJsonB = readResponse.read()
            readResponseJson = json.loads(readResponseJsonB)
...
def testServerAPIGetClusterMembers(serverURL):
    ...
    try:
        ...
        testResult = False
        notes = e.reason
        errorMsg = (f'Error creating entities!  Traceback = {notes}' )
    except Exception as e:
        testResult = "False"
        errorMsg = (f'Error creating entities!  Traceback = {e}' )
        errata.append(errorMsg)
    ...
    try:
        ...
        testResult = "False"
        errorMsg = (f'Error measuring cluster membership!  Traceback = {e}' )
        errata.append(errorMsg)
                ...
                if sanityTestCase["expectedResults"][checkNode] == False:
                    for node in sanityTestCase["traverse"]["nodes"]:
                        if checkNode == node["id"]:
                            testResult = "False"
                            errorMsg = f'Error in traverse report for {traverseDictE1toAction["query"]}!  Node found, which should not have been present'
                            errata.append(errorMsg)
                else:
                    found = False
                    for node in sanityTestCase["traverse"]["nodes"]:
                    ...
                    if found == False:
                        testResult = "False"
                        errorMsg = f'Error in traverse report for {sanityTestCase["query"]}!  Node not found, which should have been present'
                        errata.append(errorMsg)
        
    except Exception as e:
        testResult = "False"
        errorMsg = (f'Getting traverse reports!  Traceback = {e}' )
        errata.append(errorMsg)
    ...
    Graph.logQ.put( [logType , logLevel.INFO , method , f"Finished testcase {1}"])
    Graph.logQ.put( [logType , logLevel.DEBUG , method , "exiting"])
    return resultSet
...
def testServerAPIAddOwner(serverURL):
    ...
    try:
        ...
        errorMsg = str(fullerror[1])
        serverStartupStatus = f"Error in request invocation  {errorID}, {errorMsg}"
    ...
    if testResult == True:
        try:
            ...
            errorID = str(fullerror[0])
            errorMsg = str(fullerror[1])
            serverStartupStatus = f"Response code 200, but error encountered while processing response  {errorID}, {errorMsg}"
     
    # make sure that entity is for an Agent.Owner   
    if testResult == True:
        getEntityMemeTypeURL = serverURL + f"/modeling/getEntityMemeType/{ownerUUIDJson['entityUUID']}"
        try:
            #urllib GET request
            getTypeResponse = urllib.request.urlopen(getEntityMemeTypeURL)  
            getTypeResponseJsonB = getTypeResponse.read()
            ...
            if entityType != "Agent.Owner":
                testResult = False
                serverStartupStatus = f"Meme type returned = {entityType}.  Should be Agent.Owner"
        except Exception as e:
            testResult = False
            fullerror = sys.exc_info()
            errorID = str(fullerror[0])
            errorMsg = str(fullerror[1])
            serverStartupStatus = f"Owner creation Response code 200 and entityUUID returned, but error encountered while verifying entity meme type: {errorID}, {errorMsg}"
    
        
    if testResult == True:
        statusString = "Successfully created and verified Agent.Owner" 
    else:
        statusString = f"Failure!: {serverStartupStatus}"
...
def testServerAPIOwnerCallbackURL(serverURL, callbackTestServerURL):
    ...
    try:
        ...
        errorID = str(fullerror[0])
        errorMsg = str(fullerror[1])
        registerStatus = f"{registerStatus}. Error in request invocation  {errorID}, {errorMsg}"
    ...
    if testResult == True:
        try:
            ...
            if "entityUUID" not in ownerUUIDJson.keys():
                testResult = False
                registerStatus = f"{registerStatus}.  Response code 200, but entityUUID not in response"
            else:
            ...
            errorID = str(fullerror[0])
            errorMsg = str(fullerror[1])
            registerStatus = f"{registerStatus}.  Response code 200, but error encountered while processing response  {errorID}, {errorMsg}"
    ...
    if testResult == True:
        ...
        try:
            ...
            response1 = urlopen(request).read().decode('utf8')
            responseStr1= json.loads(response1)
            registerStatus = f"{registerStatus}.  All Good"
        except urllib.error.URLError as e:
            testResult = False
            fullerror = sys.exc_info()
            errorID = str(fullerror[0])
            errorMsg = str(fullerror[1])
            registerStatus = f"{registerStatus}.  urllib.error.URLError while adding stimulusCallbackURL to Agent.Owner : {errorID}, {errorMsg}"
        except Exception as e:
            testResult = False
            fullerror = sys.exc_info()
            errorID = str(fullerror[0])
            errorMsg = str(fullerror[1])
            registerStatus = f"{registerStatus}, Problem while adding stimulusCallbackURL to Agent.Owner : {errorID}, {errorMsg}"
      
    #Lastly, test it      
    if testResult != False:
        try:
            #Now read that same property    
            getPropURL = serverURL + f"/modeling/getEntityPropertyValue/{entityID}/{"stimulusCallbackURL"}"
            readResponse = urllib.request.urlopen(getPropURL) 
            readResponseJsonB = readResponse.read()
            readResponseJson = json.loads(readResponseJsonB)
...
def testServerAPICreatorCallbackURLs(serverURL, callbackTestServerURL):
    ...
    if testResult != False:
        try:
            #Now read that same property    
            getPropURL = serverURL + f"/modeling/getEntityPropertyValue/{entityID}/{"dataCallbackURL"}"
            readResponse = urllib.request.urlopen(getPropURL) 
            readResponseJsonB = readResponse.read()
            readResponseJson = json.loads(readResponseJsonB)
...
def convertIntListToString(intList):
    ...
    if type(intList) == type([]):
        for intVal in intList:
            if nnTh > 0:
                returnString = f"{returnString}, {intVal}"
            else:
                returnString = f"{returnString}{intVal}"
            nnTh = nnTh +1
    else:
        returnString = f"{returnString}{intList}"
...
def runTests(testPrefix):
    ...
    testcaseName = f"{testPrefix} - Action Engine (Action Queue)"
    ...
    testSetPercentage = getResultPercentage(testSetData)
    testcaseName = f"{testPrefix} - Action Engine (Ad Hoc Set, Single Agent)"
    resultSet.append([testcaseName, testSetPercentage, copy.deepcopy(testSetData)])  
    ...
    testSetPercentage = getResultPercentage(testSetData)
    testcaseName = f"{testPrefix} - Action Engine (Ad Hoc Set, multi-Agent)"
    resultSet.append([testcaseName, testSetPercentage, copy.deepcopy(testSetData)])  
    ...
    testSetPercentage = getResultPercentage(testSetData)
    testcaseName = f"{testPrefix} - Action Engine (Mixed Set, multi-Agent)"
    resultSet.append([testcaseName, testSetPercentage, copy.deepcopy(testSetData)])  
    ...
    testSetPercentage = getResultPercentage(testSetData)
    testcaseName = f"{testPrefix} - Action Engine (Mixed Set Choreography, multi-Agent)"
    resultSet.append([testcaseName, testSetPercentage, copy.deepcopy(testSetData)])  
    ...
    testSetPercentage = getResultPercentage(testSetData)
    testcaseName = f"{testPrefix} - Action Engine (Dynamic, Ad-Hoc Required Landmarks)"
    resultSet.append([testcaseName, testSetPercentage, copy.deepcopy(testSetData)])  
    """
    ...
    testSetPercentage = getResultPercentage(testSetData)
    testcaseName = f"{testPrefix} - Stimulus Engine (Trailer Pass Through)"
    resultSet.append([testcaseName, testSetPercentage, copy.deepcopy(testSetData)])  
    ...
    testSetPercentage = getResultPercentage(testSetData)
    testcaseName = f"{testPrefix} - Stimulus Engine (Trailer with different Agent Views)"
    resultSet.append([testcaseName, testSetPercentage, copy.deepcopy(testSetData)]) 
    ...
    testSetPercentage = getResultPercentage(testSetData)
    testcaseName = f"{testPrefix} - Stimulus Engine (Trailer with no target agents)"
    resultSet.append([testcaseName, testSetPercentage, copy.deepcopy(testSetData)])
    ...
    testSetPercentage = getResultPercentage(testSetData)
    testcaseName = f"{testPrefix} - Stimulus Engine (Conditional Stimuli, restricted agents)"
    resultSet.append([testcaseName, testSetPercentage, copy.deepcopy(testSetData)])
    ...
    testSetPercentage = getResultPercentage(testSetData)
    testcaseName = f"{testPrefix} - Stimulus Engine (Conditional Stimuli, all agents)"
    resultSet.append([testcaseName, testSetPercentage, copy.deepcopy(testSetData)])
    ...
    testSetPercentage = getResultPercentage(testSetData)
    testcaseName = f"{testPrefix} - Internationalized Descriptor"
    resultSet.append([testcaseName, testSetPercentage, copy.deepcopy(testSetData)])
...
def runRestartTests(testPrefix = "Restart"):
    ...
    testSetPercentage = getResultPercentage(testSetData)
    testcaseName = f"{testPrefix} - Engine Restart (No Reset)"
    resultSet.append([testcaseName, testSetPercentage, copy.deepcopy(testSetData)])    
    
    print("Engine Restart (Reset)")
    testSetData = testEngineRestart(True)
    testSetPercentage = getResultPercentage(testSetData)
    testcaseName = f"{testPrefix} - Engine Restart (Reset)"
    resultSet.append([testcaseName, testSetPercentage, copy.deepcopy(testSetData)])  
...
def publishResults(testReports, css, fileName, titleText, ranUnitTests, ranAPITests):
    ...
    h3Tests1Text = doc.createTextNode(f"Unit Tests Executed:  {ranUnitTests}")
    h3Tests1.appendChild(h3Tests1Text)
    body.appendChild(h3Tests1)
    
    h3Tests2 = doc.createElement("h3")
    h3Tests2Text = doc.createTextNode(f"API Tests Executed:  {ranAPITests}")
    h3Tests2.appendChild(h3Tests2Text)
    body.appendChild(h3Tests2)
    
    h2 = doc.createElement("h2")
    h2Text = doc.createTextNode(f"{totalTCCount} regression tests over {numReports} persistence types in in {totalTCTime:.1f} seconds:  {totalTCTime}")
    ...
    for testReport in testReports:
        ...
        icTableHeadText = doc.createTextNode(f"{profileName}, {localPersistenceName}: %.1f seconds" )
        ...
        for unusedI in range(0, numberOfColumns):
            ...
            for headerElement in headers:
                ...
                nestedCellText = doc.createTextNode(f"{headerElement}")
            ...
            for dummyJ in range(0, rowsPerColumn):
                ...
                if listPosition <= numberOfModules:
                    try:
                        ...
                        cellText = doc.createTextNode(f"{moduleReport}"[0])
                        hyperlinkNode = doc.createElement("a")
                        hyperlinkNode.setAttribute("href", f"#{moduleReport[0]}{localPersistenceName}") 
                        ...
                        rowData = [len(moduleReport[2]), f"{moduleReport} %%"[1]]
                        for dataEntry in rowData:
                            percentCell = doc.createElement("td")
                            percentCellText = doc.createTextNode(f"{dataEntry}")
        ...
        for testSet in resultSet:
            ...
            for headerEntry in headers:
                ...
                cellText = doc.createTextNode(f"{headerEntry}")
                cell.appendChild(cellText)
            ...
            for fullTestRow in testSet[2]:
                ...
                for dataEntry in test:
                    cell = doc.createElement("td")
                    cellText = doc.createTextNode(f"{dataEntry}")
                    cell.appendChild(cellText)
                    ...
                    try:
                        ...
                        cell = doc.createElement("td")
                        cellText = doc.createTextNode(f"Please check Testcase code: actual test result = {test[2]}, expected = {test[3]}")
                        cell.appendChild(cellText)
                        cell.setAttribute("class", "detailsCell")
                        tableRow.appendChild(cell) 
                        tableRow.setAttribute("class", "badDRow")                   
                ...
                if type(fullTestRow[4]) == type([]):
                    ...
                    for bulletpointElement in filteredErrata:
                        ...
                        pText = doc.createTextNode(f"{bulletpointElement}")
                    ...
                    filteredErrata = Graph.filterListDuplicates(fullTestRow[4])
                    paragraph = doc.createElement("p")
                    pText = doc.createTextNode(f"{filteredErrata}")
                    paragraph.appendChild(pText)
...
if __name__ == "__main__":
    ...
    if args.dbtype:
        if (args.dbtype is None) or (args.dbtype == 'none'):
            ...
            print(f"\n  -- using persistence type {args.dbtype}")
        else:
    ...
    if args.dbtcon:
        if (args.dbtcon is None) or (args.dbtcon == 'none'):
            if persistenceType is None:
                ...
                print(f"  -- Persistence type {persistenceType} requires a valid database connection.  Please provide a --dbtcon argument!")
                sys.exit()
            ...
            if persistenceType == 'sqlite':
                if dbConnectionString.endswith(".sqlite"):
                    ...
                    print(f"  -- Using sqlite persistence with invalid filename {dbConnectionString}.  It must end with the .sqlite extension")
                    sys.exit()
            else:
                print(f"  -- Using persistence type {args.dbtype} with connection = {args.dbtcon}")
    ...
    if args.repo:
        for additionalRepo in args.repo:
            ...
            print(f"  -- repo: {additionalRepo}")
    print(f"  {nRepoCount} repositories (including Memetic core,  repo) are being used")
