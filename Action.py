...
class LogProxy(object):
    ...
    def initialize(self, serverURL):
        self.serverURL = serverURL
        
    def setLogTypeEngine(self):
        self.logType = 0
        
    def setLogTypeContent(self):
        self.logType = 1
        
    def log(self, logType, logLevel, origin, message):
        pass
...
class APIProxy(object):
    ...
    def getEntityMemeType(self, entityUUID):
        ...
        return getTypeResponseJson["memeType"]
    
    def getCounterpartsByType(self, originEntityID, query, isMeme = True):
        requestURL = self.serverURL + "/modeling/query"
        if isMeme == False:
            requestURL = self.serverURL + "/modeling/querym"

        postFieldsDict = {"originEntityID" : originEntityID, "query": query}
        #urllib POST request
        request = Request(url=requestURL, data=bytes(json.dumps(postFieldsDict), encoding='utf-8'))
        queryResponse = urlopen(request).read().decode('utf8')
        try:
            queryResponseJson = json.loads(queryResponse)
        except:
            queryResponseJsonB = queryResponse.read()
            queryResponseJson = json.loads(queryResponseJsonB)          
        counterpartList = queryResponseJson["entityIDList"]
        return counterpartList
    
    def getLinkCounterpartsByMetaMemeType(self, originEntityID, query):
        counterpartList = self.getCounterpartsByType(originEntityID, query, False)
        return counterpartList
    
                
    def getHasCounterpartsByType(self, originEntityID, landMarkPath):
        counterpartList = self.getCounterpartsByType(originEntityID, landMarkPath)
        if len(counterpartList) > 0:
            return True
        else:
            return False
    ...
    def getEntityPropertyValue(self, entityUUID, propName):
        ...
        getValResponseJson = json.loads(getValResponseJsonB)
        return getValResponseJson["propertyValue"]
...
class Action(object):
    ...
    def initialize(self, uuid, actionID):
        ...
        try:
            global apiProxy
            self.uuid = uuid
            self.meme = apiProxy.getEntityMemeType(actionID)
    ...
    def inflateMembers(self, unusedScript):
        #this method is only relevant for sets
        pass       
    
    
            
    def addLandMarks(self, unusedScript):
        """
        ...
        try:
            ...
            global apiProxy
            self.landmarksNonExclusive = apiProxy.getLinkCounterpartsByMetaMemeType(self.uuid, lmNoExPath)
            self.landmarksExclusive = apiProxy.getLinkCounterpartsByMetaMemeType(self.uuid, lmExPath)
            ...
            reqLMRoots = apiProxy.getLinkCounterpartsByMetaMemeType(self.uuid, reqLMRootPath)
            for reqLMRoot in reqLMRoots:
                reqLMs = apiProxy.getLinkCounterpartsByMetaMemeType(reqLMRoot, reqLMPath)
            ...
            logProxy.log(logProxy.logType, logProxy.WARNING , method , e)
        except Exception as e:
    ...
    def mapFunctionLandmarks(self, landMarkID, agentUUID):
        ...
        try:
            landMarkPath = apiProxy.getEntityMemeType(landMarkID)
